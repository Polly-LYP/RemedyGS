...
class MIPNerf_MV_Images(Dataset):
    ...
    def __init__(self, img_per_batch=4, root="/home/ylitx/Project/25_4/RemedyGS_Apr/Our_Main/In_domain_exp/Sampled_dataset/100",
                ...
                crop_size=(1056,1600), subset="test"):
        """
        Args:
    ...
    def __getitem__(self, index):
        ...
        #get without resize image
        origin_h = images.squeeze().shape[1]
        origin_w = images.squeeze().shape[2]
        transform_with_resize = transforms.Compose([transforms.ToTensor(),transforms.Resize((origin_h,origin_w))])


        #get groundtruth image
        clean_root = "/home/ylitx/Project/25_4/RemedyGS_Apr/Main/poison-splat/dataset/MIP_Nerf_360"
        clean_image = []
        for item in image_batch:
            scene_name = item.split('/')[-3]
            image_name = item.split('/')[-1]
            image_path = os.path.join(clean_root,scene_name,'images',image_name)
            clean_image = clean_image + [transform_with_resize(Image.open(image_path).convert('RGB'))]
        clean_image = torch.stack(clean_image)
        return images,image_batch,clean_image
    ...
    def get_files(self,path,dataset_list):
        ...
        for line in lines:
            ...
            image_4_dir = Path(os.path.join(path, line,'images'))
            if image_4_dir.exists() and image_4_dir.is_dir():
                image_each_hash_list = [str(img_path) for img_path in image_4_dir.glob('*.JPG')]
