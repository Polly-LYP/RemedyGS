...
class MIPNerf_MV_Images(Dataset):
    ...
    def __init__(self,root="/home/ylitx/Project/25_4/RemedyGS_Apr/Our_Main/In_domain_exp/Sampled_dataset/100",
        ...
        if subset=="train":
            ...
            self.transform = transforms.Compose([transforms.ToTensor(),transforms.RandomCrop((crop_size[0], crop_size[1]))])
            self.crop_size = None
    ...
    def get_files(self,path,dataset_list,attacked_path):
        ...
        for line in lines:
            ...
            attacked_image_4_dir = Path(os.path.join(attacked_path, line,'images'))
            if image_4_dir.exists() and image_4_dir.is_dir():
                image_list = image_list + [str(img_path) for img_path in image_4_dir.glob('*.JPG')]
                img_count = img_count + len([str(img_path) for img_path in image_4_dir.glob('*.JPG')])
                img_label = img_label + [0 for img_path in image_4_dir.glob('*.JPG') ]

            if attacked_image_4_dir.exists() and attacked_image_4_dir.is_dir():
                image_list = image_list + [str(img_path) for img_path in attacked_image_4_dir.glob('*.JPG')]
                img_count = img_count + len([str(img_path) for img_path in attacked_image_4_dir.glob('*.JPG')])
                img_label = img_label + [1 for img_path in attacked_image_4_dir.glob('*.JPG')]
