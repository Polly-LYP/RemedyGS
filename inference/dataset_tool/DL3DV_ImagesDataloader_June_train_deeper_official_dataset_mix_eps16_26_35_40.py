...
class DL3DV_MV_Images(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # get attacked image
        attacked_root = "/home/ylitx/Project/25_4/RemedyGS_Apr/Official_dataset_Our_main/In_domain_exp/Sampled_dataset/attacked/attacker_output_sampled_100_30000_25_mix_eps16_26_35_40/100_eps16_26_35_40"
        attacked_image = []
        for image in image_batch:
            scene_name = image.split('/')[-3]
            image_name = image.split('/')[-1]
            image_path = os.path.join(attacked_root,scene_name,'images',image_name)
            attacked_image = attacked_image + [self.transform(Image.open(image_path).convert('RGB'))]
        if self.crop_size is not None:
            _, h, w = attacked_image[0].shape
            th, tw = self.crop_size
            i = (h - th) // 2
            j = (w - tw) // 2
            attacked_image = [tf.crop(image, i, j, th, tw) for image in attacked_image]        
        attacked_image = torch.stack(attacked_image)


        return images,attacked_image,scene_name,image_batch
