...
class MIPNerf_MV_Images(Dataset):
    ...
    def __getitem__(self, index):
        ...
        transform_without_resize = transforms.Compose([transforms.ToTensor()])
        without_resized_images = [transform_without_resize(Image.open(img).convert('RGB')) for img in image_batch]
        without_resized_images_shape_1 = [image.shape[1] for image in without_resized_images]
        without_resized_images_shape_2 = [image.shape[2] for image in without_resized_images]


        return images,image_batch,without_resized_images_shape_1,without_resized_images_shape_2
