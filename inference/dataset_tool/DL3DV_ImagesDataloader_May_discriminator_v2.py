...
class DL3DV_MV_Images(Dataset):
    ...
    def __getitem__(self, index):
        ...
        labels = torch.tensor(self.labels_list[index])
    ...
    def get_files(self,path,dataset_list,attacked_path):
        ...
        for line in lines:
            ...
            attacked_image_4_dir = Path(os.path.join(attacked_path, line,'images_4'))
            if image_4_dir.exists() and image_4_dir.is_dir():
                image_per_scene = [str(img_path) for img_path in image_4_dir.glob('*.png')]
                img_label_per_scene = [0 for img_path in image_4_dir.glob('*.png') ]

            if attacked_image_4_dir.exists() and attacked_image_4_dir.is_dir():
                attacked_per_scene = [str(img_path) for img_path in attacked_image_4_dir.glob('*.png')]
                attacked_img_label_per_scene = [1 for img_path in attacked_image_4_dir.glob('*.png')]
            
            total_img_per_scene = image_per_scene + attacked_per_scene
            total_img_label_per_scene = img_label_per_scene + attacked_img_label_per_scene
            
            combined = list(zip(total_img_per_scene, total_img_label_per_scene))
            random.shuffle(combined)
            total_image_list = []
            total_img_label = []
            total_image_list[:], total_img_label[:] = zip(*combined)

            for i in range(0,len(total_img_per_scene) - self.img_per_batch + 1, self.img_per_batch):
                image_list.append(total_image_list[i:i + self.img_per_batch])
                img_label.append(total_img_label[i:i + self.img_per_batch])
