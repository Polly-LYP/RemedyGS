...
class Nerf_syn_MV_Images(Dataset):
    ...
    def get_files(self,path,dataset_list):
        ...
        for line in lines:
            ...
            image_4_dir_2 = Path(os.path.join(path, line, 'test'))
            if image_4_dir.exists() and image_4_dir.is_dir() and image_4_dir_2.exists() and image_4_dir_2.is_dir():
                image_each_hash_list = [str(img_path) for img_path in image_4_dir.glob('*.png')]
                image_each_hash_list = image_each_hash_list + [str(img_path) for img_path in image_4_dir_2.glob('*.png')]
