...
class DL3DV_MV_Images(Dataset):
    ...
    def __getitem__(self, index):
        ...
        attacked_image_path = os.path.join(attacked_root,scene_name,'images_4',image_name)
        attacked_image = self.transform(Image.open(attacked_image_path).convert('RGB'))
        if self.crop_size is not None:
            _, h, w = attacked_image.shape
            th, tw = self.crop_size
            i = (h - th) // 2
            j = (w - tw) // 2
            attacked_image = tf.crop(attacked_image, i, j, th, tw)      


        return image,attacked_image,scene_name,image_path
