...
def victim_training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, exp_run):
    ...
    with torch.no_grad():
        for camid, cam in enumerate(viewpoint_stack):
            ...
            if getattr(args, "clean_dataset_root", None):
                scene_name = os.path.basename(os.path.normpath(args.source_path))
                raw_img_pth = os.path.join(
                    args.clean_dataset_root, scene_name, "images", cam.image_name + ".JPG"
                )
                if not os.path.exists(raw_img_pth):
                    raw_img_pth = os.path.join(
                        args.clean_dataset_root, scene_name, "images", cam.image_name + ".jpg"
                    )
                if os.path.exists(raw_img_pth):
                    ...
                    PSNR_views_with_raw.append(
                        psnr(raw_image.to(render_image.device), render_image).mean().item()
                    )
                    LPIPS_views_with_raw.append(
                        lpips_criterion(render_image, raw_image.to(render_image.device)).item()
                    )
                    ssim_views_with_raw.append(
                        ssim(render_image, raw_image.to(render_image.device)).item()
                    )
    del lpips_criterion
    ...
    mean_PSNR_with_raw = (
        round(sum(PSNR_views_with_raw) / len(PSNR_views_with_raw), 4)
        if PSNR_views_with_raw
        else float("nan")
    )
    mean_LPIPS_with_raw = (
        round(sum(LPIPS_views_with_raw) / len(LPIPS_views_with_raw), 4)
        if LPIPS_views_with_raw
        else float("nan")
    )
    mean_ssim_with_raw = (
        round(sum(ssim_views_with_raw) / len(ssim_views_with_raw), 4)
        if ssim_views_with_raw
        else float("nan")
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--clean_dataset_root",
        type=str,
        default=os.environ.get("CLEAN_DATASET_ROOT"),
        help="Optional path to clean MIP-NeRF 360 scenes for raw-image metrics",
    )
